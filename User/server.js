...
io.on("connection",(socket)=>{
  ...
  socket.on('disconnect',()=>{
    console.log(`user with id ${userId} disconnectedddddddddddddddd from server`)
  ...
  try {
    ...
    await consumer.run({
      eachMessage: async (result) => {
        const { response,userId,driverId,shipmentId } = JSON.parse(result.message.value);
        ...
        if (driverId.toString() in userSockets) {
          ...
          console.log("response sent to client who is connected as driverId: ", response, userId);
          if (response == "reject") {
  ...
  try{
    ...
    await consumer.run({
      eachMessage: async (result) =>{
        ...
        console.log("chat message consumed by user: ",message,"from: ",from,"to: ",to)
